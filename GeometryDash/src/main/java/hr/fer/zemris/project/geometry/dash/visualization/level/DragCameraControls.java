...
public class DragCameraControls implements Drawable, Changeable {
	...
	/**
	 * On every tick level editor will be updated but only if user is on level
	 * editor. Maybe here we will not obtain any better results
	 */
	@Override
	public void update() {
		...
		lastMousePosition = new Vector2D(mouseHandler.getMouse_x() + mouseHandler.getDeltaDrag_x(),
				mouseHandler.getMouse_y() + mouseHandler.getDeltaDrag_y());
	}
	...
	private void drawYLines(GraphicsContext graphicsContext) {
		double x_start = Math.floor(camera.getPosition().getX() / GameConstants.cell_width) * GameConstants.cell_width
	...
	private void drawXLines(GraphicsContext graphicsContext) {
		double y_start = Math.floor(camera.getPosition().getY() / GameConstants.cell_height) * GameConstants.cell_height
